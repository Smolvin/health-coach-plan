...
`.env` для прода — корневой (тот же файл, что и у локальной разработки).
`docker-compose.yml` ссылается на него как `../../.env` — `.env`
уже попадёт в контейнер вместе со всем остальным репозиторием (bind-mount
`../..:/app`), отдельно копировать/монтировать не нужно.
